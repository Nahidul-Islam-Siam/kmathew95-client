...
export default function SignUpPage() {
  ...
  const [isModalVisible, setIsModalVisible] = useState(false); // Add this
  ...
  try {
    const res = await useregister(payload).unwrap();
    if (res?.success) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center">
        ...
        {isModalVisible && <VerificationModal open={isModalVisible} onClose={() => setIsModalVisible(false)} />}
        ...
        <div className="w-full max-w-md mx-auto bg-white p-6 sm:p-8 rounded-lg shadow-md">
          ...
          <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
            <p className="text-center text-gray-600 text-base font-medium">Sign up to your account</p>
